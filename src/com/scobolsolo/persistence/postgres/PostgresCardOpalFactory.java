...
public class PostgresCardOpalFactory extends com.opal.AbstractDatabaseIdentityOpalFactory<Card, CardOpal> implements CardOpalFactory {
	...
	@Override
	protected void afterInsert(TransactionParameter argTP, CardOpal argOpal) throws PersistenceException {
		assert argTP != null;
		assert argOpal != null;
		try {
			argOpal.setId(
				com.siliconage.database.DatabaseUtility.executeIntQuery(
					...
					"SELECT last_value FROM card_id_seq AS id_value",
					null
				)
			);
			return;
		} catch (SQLException lclE) {
			throw new PersistenceException("Unable to retrieve last value for sequence column card_id_seq", lclE);
		}
	}
