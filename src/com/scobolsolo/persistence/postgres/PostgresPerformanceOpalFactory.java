...
public class PostgresPerformanceOpalFactory extends com.opal.AbstractDatabaseIdentityOpalFactory<Performance, PerformanceOpal> implements PerformanceOpalFactory {
	...
	@Override
	protected void afterInsert(TransactionParameter argTP, PerformanceOpal argOpal) throws PersistenceException {
		assert argTP != null;
		assert argOpal != null;
		try {
			argOpal.setId(
				com.siliconage.database.DatabaseUtility.executeIntQuery(
					...
					"SELECT last_value FROM performance_id_seq AS id_value",
					null
				)
			);
			return;
		} catch (SQLException lclE) {
			throw new PersistenceException("Unable to retrieve last value for sequence column performance_id_seq", lclE);
		}
	}
