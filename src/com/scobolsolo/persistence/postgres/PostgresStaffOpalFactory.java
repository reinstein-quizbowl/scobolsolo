...
public class PostgresStaffOpalFactory extends com.opal.AbstractDatabaseIdentityOpalFactory<Staff, StaffOpal> implements StaffOpalFactory {
	...
	@Override
	protected void afterInsert(TransactionParameter argTP, StaffOpal argOpal) throws PersistenceException {
		assert argTP != null;
		assert argOpal != null;
		try {
			argOpal.setId(
				com.siliconage.database.DatabaseUtility.executeIntQuery(
					...
					"SELECT last_value FROM staff_id_seq AS id_value",
					null
				)
			);
			return;
		} catch (SQLException lclE) {
			throw new PersistenceException("Unable to retrieve last value for sequence column staff_id_seq", lclE);
		}
	}
