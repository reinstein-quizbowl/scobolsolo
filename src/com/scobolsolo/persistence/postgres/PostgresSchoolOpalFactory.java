...
public class PostgresSchoolOpalFactory extends com.opal.AbstractDatabaseIdentityOpalFactory<School, SchoolOpal> implements SchoolOpalFactory {
	...
	@Override
	public SchoolOpal create(School argUF) {
		assert argUF != null;
		SchoolOpal lclOpal;
	...
	@Override
	protected void afterInsert(TransactionParameter argTP, SchoolOpal argOpal) throws PersistenceException {
		assert argTP != null;
		assert argOpal != null;
		try {
			argOpal.setId(
				com.siliconage.database.DatabaseUtility.executeIntQuery(
					...
					"SELECT last_value FROM school_id_seq AS id_value",
					null
				)
			);
			return;
		} catch (SQLException lclE) {
			throw new PersistenceException("Unable to retrieve last value for sequence column school_id_seq", lclE);
		}
	}
