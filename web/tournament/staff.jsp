...
<div class="row">
	<div class="small-12 columns">
		...
		%><table class="responsive">
			...
			<tbody><%
				Filter<SchoolRegistration> lclSchoolRegistrationRegisteredFor = new SchoolRegistrationRegisteredFor(lclT);
				...
				for (OpalForm<Staff> lclSOF : lclSOFs) {
					...
					%><tr>
						<%= lclSOF.open() %>
						<td data-tablesorter="<%= lclSOF.isNew() ? "" : lclStaff.getContact().getSortBy() %>"><%= lclSOF.dropdown("Contact", Contact.SortByComparator.getInstance()).filter(Contact.ActiveFilter.getInstance()) %></td>
						<td data-tablesorter="<%= lclSOF.isNew() ? "?" : (lclStaff.isBringingLaptop() ? 1 : 0) %>"><%= HTMLUtility.switchWidget(lclSOF, "BringingLaptop") %></td>
						<td data-tablesorter="<%= lclSOF.isNew() ? "" : (lclStaff.getSchoolRegistration() == null ? "" : lclStaff.getSchoolRegistration().getSchool().getName()) %>"><%= lclSOF.dropdown("SchoolRegistration", SchoolRegistration.SchoolShortNameComparator.getInstance()).filter(lclSchoolRegistrationRegisteredFor).namer(SchoolRegistrationNCE.getInstance()) %></td>
