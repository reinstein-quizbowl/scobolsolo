	...
	<div class="small-12 columns"><%
		...
		if (lclT.getSchoolRegistrationCount() == 0) {
			...
			for (SchoolRegistration lclSR : lclSRs) {
				...
				%><h2 id="<%= lclSR.getId() %>"><a class="no-highlight" href="player-detail.jsp?object=<%= lclT.getUniqueString() %>#school_<%= lclSR.getSchool().getId() %>"><%= lclSR.getSchool().getExplainedName() %></a></h2>
				<ul><%
					...
					for (Player lclP : lclPlayers) {
						%><li>
							<a class="no-highlight" href="player-detail.jsp?object=<%= lclT.getUniqueString() %>#player_<%= lclP.getId() %>"><%= lclP.getContact().getName() %></a><%
